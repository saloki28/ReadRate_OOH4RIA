...
{
        System.Collections.Generic.IList<ReadRate_e4Gen.ApplicationCore.EN.ReadRate_E4.AdministradorEN> result;
        try
        {
                SessionInitializeTransaction ();
                //String sql = @"FROM AdministradorNH self where FROM AdministradorNH administrador where lower(administrador.Email) like concat('%', lower(:p_email), '%')";
                //IQuery query = session.CreateQuery(sql);
                IQuery query = (IQuery)session.GetNamedQuery ("AdministradorNHdameAdministradoresPorEmailHQL");
                query.SetParameter ("p_email", p_email);

                result = query.List<ReadRate_e4Gen.ApplicationCore.EN.ReadRate_E4.AdministradorEN>();
                SessionCommit ();
        }
